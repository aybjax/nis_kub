...
func TestTransformNotifyListener(t *testing.T) {
	...
	driver.EXPECT().Subscribe(_QUEUE_CHANNEL_TO_MODIFY,
		_QUEUE_TOPIC_COURSE_TO_MODIFY_QUEUE, gomock.Any(),
...
func TestNotifyNotifyCourseChanged(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	driver := mock_cmntypes.NewMockAppQueue(ctrl)
	queue := &QueueImpl{driver}
	...
	driver.EXPECT().Publish(
		bs,
		_QUEUE_CHANNEL_COURSE_MODIFIED,
		_QUEUE_TOPIC_COURSE_MODIFIED,
	)
	queue.NotifyCourseChanged(payload)
}

func TestStudentChangedListener(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	driver := mock_cmntypes.NewMockAppQueue(ctrl)
	queue := &QueueImpl{driver}
	...
	driver.EXPECT().
		Subscribe(_QUEUE_CHANNEL_STUDENT_MODIFIED, _QUEUE_TOPIC_STUDENT_MODIFIED, gomock.Any()).
		Do(func(_, _ string, listenerCb func(data []byte) error) {
			listenerCb(bs)
		})
	queue.StudentChangedListener(cb)
}
